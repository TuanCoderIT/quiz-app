...
import { fetchUser, loginApi, logoutApi, registerApi } from "./api";
import { AuthState } from "./types"; // Sử dụng interface AuthState chuẩn của Mobile
    ...
    (set, get) => ({
      ...
      logout: async () => {
        ...
        try {
          ...
          console.log(
            "Lỗi gọi API logout (vẫn tiến hành xóa state dưới client):",
            err,
          );
        } finally {
      ...
      initializeAuth: async () => {
        // Đổi tên từ bootstrap thành initializeAuth để khớp với interface AuthState
